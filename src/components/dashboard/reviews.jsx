...
const Avatar = ({ name, src, alt }) => {
  ...
  return (
    <div className="w-10 h-10 inline-flex items-center justify-center rounded-full bg-gray-800 text-indigo-400 mb-4 overflow-hidden">
      {src ? (
            ...
            {reviews?.map((review) => (
              <div className="p-4 md:w-1/2 w-full ">
                <div className="h-full bg-gray-800 bg-opacity-40 p-8 rounded">
                  ...
                  <p className="leading-relaxed mb-3">{review.comment}</p>
                  <p className="leading-relaxed mb-6 flex items-center space-x-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`h-5 w-5 ${
